...
its row's only block. The centered video renders at its intrinsic aspect ratio
(height-capped, so portrait videos narrow rather than overflow) with the full
Media Control Bar; previous/next videos peek at the edges, dimmed and scaled
down so the current video sits slightly forward. Scroll-snap browsing with
Card Carousel header controls (page-inset so they align to the grid),
