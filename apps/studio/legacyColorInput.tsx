...
const getHue = (red: number, green: number, blue: number, maximum: number, delta: number) => {
  if (delta === 0) return 0
  if (maximum === red) return (((green - blue) / delta + 6) % 6) * 60
  if (maximum === green) return ((blue - red) / delta + 2) * 60
...
const colorFromHex = (hex: string) => {
  ...
  const hue = getHue(red, green, blue, maximum, delta)
  ...
  return {
    ...
    hsl: {
      _type: 'hslaColor',
      h: hue,
      s: delta === 0 ? 0 : clamp(delta / (1 - Math.abs(2 * lightness - 1))),
      l: clamp(lightness),
      a: 1,
    },
    hsv: {
